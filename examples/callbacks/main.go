...
func demoSuccess() {
	...
	_ = proxy.Register(func(cli *bun.DB) error {
		_, err := cli.ExecContext(context.Background(),
			"INSERT INTO app_meta (key, value) VALUES (?, ?)", "version", "v3")
		return err
	})

	// WithParallelCallback 让回调并行执行,但 Init 会等待全部回调结束后才返回
	err := proxy.Init("sqlite://:memory:", bunproxy.WithParallelCallback())
	if err != nil {
